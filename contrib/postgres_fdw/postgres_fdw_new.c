...
/* Keep parameterized-scan caching bounded per ForeignScan. */
#define PGFDW_PARAM_CACHE_MAX_ENTRIES	1024
...
enum FdwScanPrivateIndex
{
	...
	/* SEMIJOIN OPTIMIZATION: Flag indicating semijoin candidate detected */
	FdwScanPrivateSemijoinActive,
	/* SEMIJOIN OPTIMIZATION: Integer attribute number of join key column */
	FdwScanPrivateSemijoinKeyAttnum,
	/* SEMIJOIN OPTIMIZATION: Oid of join key data type */
...
typedef struct PgFdwScanState
{
	...
	/* SEMIJOIN OPTIMIZATION state */
	bool		semijoin_candidate;	/* planner found a semijoin opportunity */
	AttrNumber	semijoin_key_attnum; /* attribute number of join key */
	Oid			semijoin_key_type;	/* data type of join key */
	int			semijoin_batch_size; /* keys per remote query (default 500) */
	MemoryContext semijoin_cxt;	/* dedicated context for key buffers */

	/*
	 * Parameterized-scan result cache.  This is a safe executor-local
	 * approximation of batched semijoin filtering for repeated local keys:
	 * once all rows for a parameter set have been fetched, later rescans with
	 * the same key can skip the remote round trip.
	 */
	bool		param_cache_enabled;
	bool		param_cache_scan;
	MemoryContext param_cache_cxt;
	List	   *param_cache_entries;
	int			param_cache_entry_count;
	char	   *param_cache_pending_key;
	List	   *param_cache_pending_tuples;
	int			param_cache_pending_count;
	int			param_cache_hits;
	int			param_cache_misses;
} PgFdwScanState;

typedef struct PgFdwParamCacheEntry
{
	char	   *key;
	HeapTuple  *tuples;
	int			num_tuples;
} PgFdwParamCacheEntry;
...
static bool ec_member_matches_foreign(PlannerInfo *root, RelOptInfo *rel,
									  ...
									  void *arg);
static bool detect_semijoin_candidate(PlannerInfo *root, RelOptInfo *baserel);
static bool detect_semijoin_clause(PlannerInfo *root, RelOptInfo *baserel,
								   RestrictInfo *rinfo);
static List *build_semijoin_implied_remote_conds(PlannerInfo *root,
												 RelOptInfo *foreignrel);
static char *build_param_cache_key(PgFdwScanState *fsstate);
static PgFdwParamCacheEntry *lookup_param_cache_entry(PgFdwScanState *fsstate,
													  const char *key);
static void begin_param_cache_fill(PgFdwScanState *fsstate, char *key);
static void remember_param_cache_tuple(PgFdwScanState *fsstate,
									   HeapTuple tuple);
static void finish_param_cache_fill(PgFdwScanState *fsstate);
static void reset_param_cache_fill(PgFdwScanState *fsstate);
						  ...
						  Oid foreigntableid)
{
	...
	fpinfo->semijoin_eligible = false;
	fpinfo->semijoin_key = NULL;
	fpinfo->semijoin_local_key = NULL;
	fpinfo->semijoin_threshold = 500;
                        ...
                        Oid foreigntableid)
{
	...
	/*
	 * Phase-1 semijoin detection: remember simple local-foreign integer
	 * equi-join keys so planning and EXPLAIN can surface the opportunity.
	 * The executor side intentionally remains on the normal parameterized
	 * scan path until a safe key-collection plan node exists.
	 */
	fpinfo->semijoin_eligible = detect_semijoin_candidate(root, baserel);
					   ...
					   Plan *outer_plan)
{
	...
	if (IS_SIMPLE_REL(foreignrel))
	{
		 ...
		 */
		foreach(lc, build_semijoin_implied_remote_conds(root, foreignrel))
		{
			Expr	   *implied_expr = (Expr *) lfirst(lc);

			if (!list_member(remote_exprs, implied_expr))
				remote_exprs = lappend(remote_exprs, implied_expr);
		}
	...
	fdw_private = lappend(fdw_private,
						  makeBoolean(fpinfo->semijoin_eligible));
	if (fpinfo->semijoin_key)
	{
		fdw_private = lappend(fdw_private,
							  makeInteger(fpinfo->semijoin_key->varattno));
		fdw_private = lappend(fdw_private,
							  makeInteger(fpinfo->semijoin_key->vartype));
	}
	else
	{
		fdw_private = lappend(fdw_private,
...
postgresBeginForeignScan(ForeignScanState *node, int eflags)
{
	...
	if (numParams > 0 && !fsstate->async_capable)
	{
		fsstate->param_cache_enabled = true;
		fsstate->param_cache_cxt = AllocSetContextCreate(
			estate->es_query_cxt,
			"postgres_fdw parameterized scan cache",
			ALLOCSET_DEFAULT_SIZES);
	}
	
	/* Initialize semijoin planning metadata carried through fdw_private. */
	if (fsplan->fdw_private && list_length(fsplan->fdw_private) > FdwScanPrivateSemijoinActive)
	{
		bool semijoin_candidate = boolVal(list_nth(fsplan->fdw_private,
												   FdwScanPrivateSemijoinActive));
		
		if (semijoin_candidate)
		{
			fsstate->semijoin_candidate = true;
			fsstate->semijoin_key_attnum = intVal(list_nth(fsplan->fdw_private,
										  FdwScanPrivateSemijoinKeyAttnum));
			fsstate->semijoin_key_type = intVal(list_nth(fsplan->fdw_private,
										   FdwScanPrivateSemijoinKeyType));
			
			/* Default batch size: 500 keys per remote query */
			fsstate->semijoin_batch_size = 500;
			
			/* Create memory context for semijoin key buffers */
			fsstate->semijoin_cxt = AllocSetContextCreate(
				estate->es_query_cxt,
				"postgres_fdw semijoin keys",
				ALLOCSET_SMALL_SIZES);
		}
	}
	else
	{
		fsstate->semijoin_candidate = false;
	}
}
...
postgresIterateForeignScan(ForeignScanState *node)
{
	 ...
	 * In async mode, we would have already created the cursor before we get
	 * here, even if this is the first call after Begin or ReScan.
	 */
	if (!fsstate->cursor_exists && !fsstate->param_cache_scan)
		create_cursor(node);
...
postgresReScanForeignScan(ForeignScanState *node)
{
	...
	if (fsstate->param_cache_scan)
	{
		...
		fsstate->param_cache_scan = false;
		fsstate->tuples = NULL;
	...
	reset_param_cache_fill(fsstate);
	fsstate->tuples = NULL;
	fsstate->num_tuples = 0;
	fsstate->next_tuple = 0;
	fsstate->fetch_ct_2 = 0;
...
postgresEndForeignScan(ForeignScanState *node)
{
	...
	/* Clean up semijoin metadata memory context. */
	if (fsstate->semijoin_cxt)
	{
		MemoryContextDelete(fsstate->semijoin_cxt);
		fsstate->semijoin_cxt = NULL;
	}

	if (fsstate->param_cache_cxt)
	{
		MemoryContextDelete(fsstate->param_cache_cxt);
		fsstate->param_cache_cxt = NULL;
	}
...
postgresExplainForeignScan(ForeignScanState *node, ExplainState *es)
{
	...
	if (plan->scan.scanrelid == 0 &&
		list_length(fdw_private) > FdwScanPrivateRelations)
	{
	...
	if (es->analyze && fsstate && fsstate->semijoin_candidate &&
		fsstate->param_cache_enabled &&
		list_length(fdw_private) > FdwScanPrivateSemijoinActive)
	{
		int			key_attnum;
		Oid			key_type;

		key_attnum = intVal(list_nth(fdw_private,
									 FdwScanPrivateSemijoinKeyAttnum));
		key_type = (Oid) intVal(list_nth(fdw_private,
										 FdwScanPrivateSemijoinKeyType));

		ExplainPropertyText("Semijoin Pushdown",
							"candidate (one-column integer equi-join)", es);
		ExplainPropertyInteger("Semijoin Key Attnum", NULL, key_attnum, es);
		ExplainPropertyInteger("Semijoin Batch Size", NULL, 500, es);
		if (es->verbose)
			ExplainPropertyInteger("Semijoin Key Type", NULL, key_type, es);
	}

	if (fsstate && fsstate->param_cache_enabled)
	{
		ExplainPropertyInteger("Parameterized Cache Hits", NULL,
							   fsstate->param_cache_hits, es);
		ExplainPropertyInteger("Parameterized Cache Misses", NULL,
							   fsstate->param_cache_misses, es);
		ExplainPropertyInteger("Parameterized Cache Entries", NULL,
							   fsstate->param_cache_entry_count, es);
	}
}
						...
						Cost *p_startup_cost, Cost *p_total_cost)
{
	...
	/* Apply network penalty for large data transfers to incentivize semijoin */
	if (retrieved_rows > 5000)
	{
		double network_penalty = (retrieved_rows - 5000) * 0.002;
		*p_total_cost += network_penalty;
	}
 ...
 *
 * Phase-1 detector for the local-foreign semijoin prefilter idea.  We only
 * recognize one foreign-table INT4 column joined to some other relation by a
 * merge/hash-joinable binary operator.  This deliberately records metadata
 * only; executing batched remote probes needs a separate plan node so that
 * collecting local keys does not disturb the join executor's outer plan.
 */
static bool
detect_semijoin_candidate(PlannerInfo *root, RelOptInfo *baserel)
{
	ListCell   *lc;
	...
	foreach(lc, baserel->joininfo)
	{
		...
		if (detect_semijoin_clause(root, baserel, rinfo))
			return true;
	}
	...
	if (baserel->has_eclass_joins)
	{
		...
		foreach(lc1, root->eq_classes)
		{
			...
			{
				EquivalenceMember *foreign_em = (EquivalenceMember *) lfirst(lc2);
				Expr	   *foreign_expr = foreign_em->em_expr;
				ListCell   *lc3;

				while (foreign_expr && IsA(foreign_expr, RelabelType))
					foreign_expr = ((RelabelType *) foreign_expr)->arg;

				if (!IsA(foreign_expr, Var) ||
					((Var *) foreign_expr)->varno != baserel->relid ||
					((Var *) foreign_expr)->varattno <= 0 ||
					((Var *) foreign_expr)->vartype != INT4OID)
					continue;

				foreach(lc3, ec->ec_members)
				{
					PgFdwRelationInfo *fpinfo = (PgFdwRelationInfo *) baserel->fdw_private;
					EquivalenceMember *local_em = (EquivalenceMember *) lfirst(lc3);
					Expr	   *local_expr = local_em->em_expr;
					RelOptInfo *localrel;

					if (local_em == foreign_em)
						continue;

					while (local_expr && IsA(local_expr, RelabelType))
						local_expr = ((RelabelType *) local_expr)->arg;

					if (!IsA(local_expr, Var) ||
						((Var *) local_expr)->vartype != ((Var *) foreign_expr)->vartype)
						continue;

					if (((Var *) local_expr)->varno <= 0 ||
						((Var *) local_expr)->varno >= root->simple_rel_array_size)
						continue;

					localrel = root->simple_rel_array[((Var *) local_expr)->varno];
					if (localrel == NULL || localrel->fdw_private != NULL)
						continue;

					fpinfo->semijoin_key = (Var *) foreign_expr;
					fpinfo->semijoin_local_key = (Var *) local_expr;
					return true;
				}
			}
		}
...
static bool
detect_semijoin_clause(PlannerInfo *root, RelOptInfo *baserel,
					   RestrictInfo *rinfo)
{
	PgFdwRelationInfo *fpinfo = (PgFdwRelationInfo *) baserel->fdw_private;
	OpExpr	   *opexpr;
	...
	if (IsA(left, Var) &&
		((Var *) left)->varno == baserel->relid &&
		((Var *) left)->varattno > 0)
	{
		foreign_var = (Var *) left;
		other_expr = right;
	}
	else if (IsA(right, Var) &&
			 ((Var *) right)->varno == baserel->relid &&
			 ((Var *) right)->varattno > 0)
	{
		foreign_var = (Var *) right;
		other_expr = left;
	}

	if (foreign_var == NULL || !IsA(other_expr, Var))
		return false;

	local_var = (Var *) other_expr;

	if (foreign_var->vartype != INT4OID ||
		local_var->vartype != foreign_var->vartype)
		return false;

	if (local_var->varno <= 0 ||
		local_var->varno >= root->simple_rel_array_size)
		return false;

	localrel = root->simple_rel_array[local_var->varno];
	if (localrel == NULL || localrel->fdw_private != NULL)
		return false;

	if (!op_mergejoinable(opexpr->opno, foreign_var->vartype) &&
		!op_hashjoinable(opexpr->opno, foreign_var->vartype))
		return false;

	other_relids = pull_varnos(root, other_expr);
	if (bms_is_empty(other_relids) ||
		bms_overlap(other_relids, baserel->relids))
	{
		bms_free(other_relids);
		return false;
	}

	fpinfo->semijoin_key = foreign_var;
	fpinfo->semijoin_local_key = local_var;
	bms_free(other_relids);
	return true;
}

/*
 * Build remote quals implied by a local restriction on the matching join key.
 * For an inner join like:
 *
 *   remote.customer_id = local.customer_id AND local.customer_id < 1000
 *
 * we can safely add:
 *
 *   remote.customer_id < 1000
 *
 * to the remote scan.  This is the phase-1 semijoin win for selective local
 * key predicates: fewer remote rows cross the wire while the normal local
 * join still proves correctness.
 */
static List *
build_semijoin_implied_remote_conds(PlannerInfo *root, RelOptInfo *foreignrel)
{
	PgFdwRelationInfo *fpinfo = (PgFdwRelationInfo *) foreignrel->fdw_private;
	Var		   *foreign_key;
	Var		   *local_key;
	...
	if (!fpinfo->semijoin_eligible ||
		fpinfo->semijoin_key == NULL ||
		fpinfo->semijoin_local_key == NULL)
		return NIL;

	foreign_key = fpinfo->semijoin_key;
	local_key = fpinfo->semijoin_local_key;

	if (local_key->varno <= 0 ||
		local_key->varno >= root->simple_rel_array_size)
		return NIL;

	localrel = root->simple_rel_array[local_key->varno];
	if (localrel == NULL)
		return NIL;

	foreach(lc, localrel->baserestrictinfo)
	{
		RestrictInfo *rinfo = lfirst_node(RestrictInfo, lc);
		OpExpr	   *opexpr;
		Node	   *left;
		...
		bool		key_on_right = false;
		Relids		other_relids;
		OpExpr	   *remote_op;

		if (rinfo->pseudoconstant || !is_opclause(rinfo->clause))
			continue;
		...
		key_on_left = equal(left, local_key);
		key_on_right = equal(right, local_key);
		if (!key_on_left && !key_on_right)
			continue;

		other_relids = pull_varnos(root, key_on_left ? right : left);
		if (!bms_is_empty(other_relids))
		{
			bms_free(other_relids);
			continue;
		...
		remote_op = (OpExpr *) copyObject(opexpr);
		if (key_on_left)
			remote_op->args = list_make2(copyObject(foreign_key),
										 copyObject(lsecond(opexpr->args)));
		else
			remote_op->args = list_make2(copyObject(linitial(opexpr->args)),
										 copyObject(foreign_key));

		if (is_foreign_expr(root, foreignrel, (Expr *) remote_op))
			remote_conds = lappend(remote_conds, remote_op);
	}
...
static char *
build_param_cache_key(PgFdwScanState *fsstate)
{
	StringInfoData buf;
	int			i;

	initStringInfo(&buf);
	for (i = 0; i < fsstate->numParams; i++)
	{
		const char *value = fsstate->param_values[i];

		if (value == NULL)
			appendStringInfoString(&buf, "-1:");
		else
			appendStringInfo(&buf, "%zu:%s", strlen(value), value);
...
static PgFdwParamCacheEntry *
lookup_param_cache_entry(PgFdwScanState *fsstate, const char *key)
{
	ListCell   *lc;

	foreach(lc, fsstate->param_cache_entries)
	{
		PgFdwParamCacheEntry *entry = (PgFdwParamCacheEntry *) lfirst(lc);
...
static void
begin_param_cache_fill(PgFdwScanState *fsstate, char *key)
{
	MemoryContext oldcontext;

	reset_param_cache_fill(fsstate);

	if (!fsstate->param_cache_enabled ||
		fsstate->param_cache_entry_count >= PGFDW_PARAM_CACHE_MAX_ENTRIES)
	{
		pfree(key);
		return;
	}

	oldcontext = MemoryContextSwitchTo(fsstate->param_cache_cxt);
	fsstate->param_cache_pending_key = pstrdup(key);
	fsstate->param_cache_pending_tuples = NIL;
	fsstate->param_cache_pending_count = 0;
...
static void
remember_param_cache_tuple(PgFdwScanState *fsstate, HeapTuple tuple)
{
	MemoryContext oldcontext;

	if (fsstate->param_cache_pending_key == NULL)
		return;

	oldcontext = MemoryContextSwitchTo(fsstate->param_cache_cxt);
	fsstate->param_cache_pending_tuples =
		lappend(fsstate->param_cache_pending_tuples, heap_copytuple(tuple));
	fsstate->param_cache_pending_count++;
	MemoryContextSwitchTo(oldcontext);
}

static void
finish_param_cache_fill(PgFdwScanState *fsstate)
{
	PgFdwParamCacheEntry *entry;
	HeapTuple  *tuples;
	ListCell   *lc;
	MemoryContext oldcontext;
	int			i;
	...
	oldcontext = MemoryContextSwitchTo(fsstate->param_cache_cxt);
	entry = (PgFdwParamCacheEntry *) palloc0(sizeof(PgFdwParamCacheEntry));
	entry->key = fsstate->param_cache_pending_key;
	entry->num_tuples = fsstate->param_cache_pending_count;
	tuples = (HeapTuple *) palloc0(sizeof(HeapTuple) *
								   Max(entry->num_tuples, 1));

	i = 0;
	foreach(lc, fsstate->param_cache_pending_tuples)
		tuples[i++] = (HeapTuple) lfirst(lc);

	entry->tuples = tuples;
	fsstate->param_cache_entries = lappend(fsstate->param_cache_entries, entry);
	fsstate->param_cache_entry_count++;
	MemoryContextSwitchTo(oldcontext);

	fsstate->param_cache_pending_key = NULL;
	fsstate->param_cache_pending_tuples = NIL;
	fsstate->param_cache_pending_count = 0;
}

static void
reset_param_cache_fill(PgFdwScanState *fsstate)
{
	fsstate->param_cache_pending_key = NULL;
	fsstate->param_cache_pending_tuples = NIL;
	fsstate->param_cache_pending_count = 0;
...
create_cursor(ForeignScanState *node)
{
	...
	if (fsstate->param_cache_enabled)
	{
		...
		cache_key = build_param_cache_key(fsstate);
		entry = lookup_param_cache_entry(fsstate, cache_key);
		if (entry)
		{
			fsstate->param_cache_hits++;
			fsstate->param_cache_scan = true;
			fsstate->cursor_exists = false;
			fsstate->tuples = entry->tuples;
			fsstate->num_tuples = entry->num_tuples;
			fsstate->next_tuple = 0;
			fsstate->fetch_ct_2 = 2;
			fsstate->eof_reached = true;
			pfree(cache_key);
			return;
		}

		fsstate->param_cache_misses++;
		begin_param_cache_fill(fsstate, cache_key);
	}
...
fetch_more_data(ForeignScanState *node)
{
	...
	PG_TRY();
	{
		...
		for (i = 0; i < numrows; i++)
		{
			...
			tuple = make_tuple_from_result_row(res, i,
											   fsstate->rel,
											   fsstate->attinmeta,
											   fsstate->retrieved_attrs,
											   node,
											   fsstate->temp_cxt);
			fsstate->tuples[i] = tuple;
			remember_param_cache_tuple(fsstate, tuple);
		}
		...
		if (fsstate->eof_reached)
			finish_param_cache_fill(fsstate);
	}
	PG_FINALLY();
	{
		PQclear(res);
